    ...
    <v-tooltip activator="parent" location="top">
      {{
        DisabledRule ? $t('Message.App.TheCurrentEnvironmentIsUnavailable') :
          $t('Message.Components.Account.LoginWithGoogle')
      }}
    </v-tooltip>
...
export default {
  name: "common-account-components-oauth-google-button",
  data: () => ({
    oauths_link: '',
  }),
  computed: {
  ...
  methods: {
    async GetOauthLink() {
      this.oauths_link = await GetOauthLink('google')
      // console.log(this.oauths_link)
      this.$forceUpdate();
    },
    Onclick() {
