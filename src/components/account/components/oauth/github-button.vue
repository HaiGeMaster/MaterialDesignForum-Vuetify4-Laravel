<template>
  <v-btn  v-show="oauths_link" variant="text" icon @click="Onclick"  >
    ...
    <v-tooltip activator="parent" location="top">
      {{
      DisabledRule ? $t('Message.App.TheCurrentEnvironmentIsUnavailable') : $t('Message.Components.Account.LoginWithGithub')
      }}
    </v-tooltip>
...
export default {
  name: "common-account-components-oauth-github-button",
  data: () => ({
    oauths_link: '',
  }),
  computed: {
  ...
  methods: {
    async GetOauthLink() {
      this.oauths_link = await GetOauthLink('github')
      this.$forceUpdate();
    },
    Onclick() {
