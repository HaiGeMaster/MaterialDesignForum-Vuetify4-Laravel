<template>
  <v-dialog max-width="500">
    <template v-slot:activator="{ props: activatorProps }">
      <v-list-item v-bind="activatorProps" prepend-icon="mdi-palette" :title="$t('Message.Components.SettingNaviagtionDrawer.Theme')"
        append-icon="mdi-dots-vertical"></v-list-item>
    </template>

    <template v-slot:default="{ isActive }">
      <v-card rounded="lg" >
        <v-toolbar :title="$t('Message.Components.SettingNaviagtionDrawer.ThemeSelection')">
        ...
        <v-card-text>
          <v-card-subtitle>
            <!-- 选择您喜欢的主题配色 -->
            {{ $t('Message.Components.SettingNaviagtionDrawer.SelectYourLikeTheme') }}
          </v-card-subtitle>
...
export default {
  ...
  methods: {
    ...
    InitTheme() {
      // 初始化主题
      let localStorageTheme = this.localDataStore.getThemeColor
  ...
  watch: {
    currentTheme(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.localDataStore.setThemeColor(newVal)
        const _this = this;
        // document.documentElement.style.setProperty('--x', event.clientX + 'px')
        // document.documentElement.style.setProperty('--y', event.clientY + 'px')
        if (document.startViewTransition) {
