<template>
  <v-card rounded="lg" :class="[
    ...
    <v-text-field color="primary"   class="mdui-textfield-input"
      :label="label || $t('Message.Components.Comments.NewCommentOrReply.WriteYourComment')" variant="outlined"
      ...
      " @focus="$emit('on_focus')" @blur="$emit('on_blur')">

      <template v-slot:append>
        <v-btn rounded="lg"  class="submit" color="primary" :loading="is_loading" @click="OnSubmit()">
    ...
    <!-- <v-btn rounded="lg"  class="submit" color="primary">
      发布
    </v-btn> -->
  </v-card>
</template>
